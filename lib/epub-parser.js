...
EpubParser = (function() {
  ...
  function extractText(filename) {
    var file = zip.file(zipFilesPrefix + filename);
  ...
  // some packages contain one root folder which contains META-INF/ and all the other
  // stuff we're interested in, detect this and set the zipFilesPrefix accordingly
  function detectZipFilesPrefix() {
    var uniqueRootFolders = {};
    var filePaths = Object.keys(zip.files);

    for(var i = 0; i < filePaths.length; i++) {
      uniqueRootFolders[filePaths[i].split('/')[0]] = true;
    }
    var uniqueRootFoldersList = Object.keys(uniqueRootFolders);

    if (uniqueRootFoldersList.length === 1) {
      zipFilesPrefix = uniqueRootFoldersList[0] + '/';
    }
  }  
    ...
    function parseContainer(err, containerJSON, finalCallback) {
      ...
      parser.parseString(opfDataXML.toString(), function(err, opfJSON) {
        ...
        if (!ncxId) { // assume epub 3 navigation doc
          ...
          var allItems = manifest[opfPrefix+"item"];

          // Fix for epub which used "opf:" prefix for only one of the item elements:
          if (opfPrefix !== 'opf' && manifest['opf:item']) {
            allItems = allItems.concat(manifest['opf:item']);
          }

          for (itemKey in allItems) {
            var item = allItems[itemKey];
            if(item["$"].id == ncxId) {
              ncxPath = opsRoot + item["$"].href;
            }
          }
          ...
          parser.parseString(ncxDataXML.toString(), function(err, ncxJSON) {
            ...
            for (prop in ncxJSON) {
              if (prop === '$') { // normal parse result
                setPrefix(ncxJSON);
              } else if (typeof ncxJSON[prop]['$'] !== 'undefined') {
