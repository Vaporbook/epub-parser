	...
	function open(filename, cb) {
		...
		*/
		...
		function readAndParseData(/* Buffer */ data, cb) {
			...
			if(!zip) cb(new Error('Parsing error'));

			for(var key in zip.files) {
				if (zip.files.hasOwnProperty(key)) {
					if(!zip.files[key].dir) fileCount++;
				}
			}
			...
			try {
				...
				// Try read custom json
				var customData = zip.file('META-INF/custom.json');
				if(customData) customJSON = JSON.parse(customData.asText());
		        ...
		        //  console.log('epub version:'+epubVersion);
			      ...
			      if(!ncxId) { // assume epub 3 navigation doc
			          ...
			          ncxDataXML = extractText(ncxPath);
								// some TOC lables has formating xml tags. This has to be removed.
								ncxDataXML = removeTagInText(ncxDataXML);
			          ...
			          parser.parseString(ncxDataXML.toString(), function (err, ncxJSON) {
		        			...
		        			function setPrefix(ncxJSON) {
										for(att in ncxJSON["$"]) {
										//console.log(att);
										if(ncxJSON["$"][att]=='http://www.daisy.org/z3986/2005/ncx/' && att == 'xmlns'){
											ncxPrefix = '';
											return;
										} else if(att.match(/^xmlns\:/)) {
		...
		function processNavPoint(np) {
			...
			if(typeof np.navPoint !== 'undefined') {
				...
				for(var i = 0; i < safeAccess(np.navPoint).length; i++) {
					obj.subitems.push(processNavPoint(np.navPoint[i]));
				}
				htmlNav += '</ul>'+"\n";
		...
		function removeTagInText(xmlStr) {
 	   var nstr = xmlStr;
 	   var len = nstr.length;
 	   var regex = /(<([^>]+)>)/ig;
 	   var start = 0;
 	   while(start < len){
 	     var firstIndex = nstr.indexOf('<text>',start);
 	     if(firstIndex > 0){
 	       var slice = nstr.slice(firstIndex+6, nstr.indexOf('</text>',firstIndex+6));
 	       var encd = slice.replace(regex,'');
 	       nstr = nstr.replace(slice,encd);
 	       len = nstr.length;
 	       start = firstIndex+encd.length+13;
 	     } else {
 	       break;
 	     }
 	   }
		...
		function buildLinearSpine() {
			for(itemref in itemreflist) {
				...
				if((itemreflist[itemref].$.linear=='yes' || typeof itemreflist[itemref].$.linear == 'undefined') &&  itemHashById[id]) {
